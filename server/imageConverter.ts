  ...
  convertAndHost: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        data = XLSX.utils.sheet_to_json<string[]>(worksheet, {
          header: 1,
          defval: "",
        });
      } catch (e) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Could not parse the Excel file. Make sure it is a valid .xlsx file.",
        });
      }
      ...
      if (imageUrls.length === 0) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "No image URLs found. Ensure columns are named MainImage, Image2 … Image8.",
        });
      }
      ...
      for (const rawUrl of imageUrls) {
        ...
        try {
          ...
          try {
            hostedUrl = await s3Put(storageKey, convertedBuffer, contentType);
          } catch (storageErr) {
            const msg = storageErr instanceof Error ? storageErr.message : String(storageErr);
            // Missing credentials = config error, surface immediately rather than silently failing all images
